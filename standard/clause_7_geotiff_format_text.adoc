...
To increase performance, the IFD should be arranged in a particular order expressed in the following recommendation. A IFD is linked to the next IFD by specifying the offset of the next IFD. In the following recommendation, the "order" implies both the order of the IFD links as well as the position in the file implied by the offset.
